...
public class AlloDraggableButton extends RelativeLayout
    implements View.OnTouchListener, Cloneable, View.OnClickListener {
  ...
  private static final int PERCENTS_OF_THRESHOLD = 100;
  // how many percents should view go to show HUD (global, local)
        ...
        .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
          @Override public void onGlobalLayout() {
            ...
            if (mY_initial_position == null) {
              ...
              Log.d(TAG, "OnGlobalLayoutListener() called with:  X="
                  + mX_initial_position
                  + "; Y="
                  + mY_initial_position);
              Log.d(TAG, "OnGlobalLayoutListener() called with:  mMediumHighestPoint ="
                  + mMediumHighestPoint
                  + ";   mMediumLowestPoint="
                  + mMediumLowestPoint);
    ...
    switch (event.getActionMasked()) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (clickDuration < MAX_CLICK_DURATION && isDistanceOfSwipeShort) {
          ...
          Log.d("Clicked",
              "onTouch: clicked; DISTANCE = " + distanceSwipe(mActionDownX, mActionDownY,
  ...
  private void checkFriendsMakeBold(Float xNewOfFAB) {
    if (isFingerOnTheRightSide(xNewOfFAB)) { // friends
      changeTextViewTypeFaceStyle(mTvFriends, Typeface.BOLD);
    } else { // local
      changeTextViewTypeFaceStyle(mTvFriends, Typeface.NORMAL);
    }
  }
  ...
  private void restoreInitialX_Y(Float yNewOfFAB) {
    ...
    mFab.animate()
        .y(AlloDraggableButton.mY_initial_position)
  ...
  private void tutorialSlideToCancel() {
    ...
    ViewPropertyAnimator animator = mFab.animate()
        .y(mY_initial_position)
        .setDuration(durationFAB)
  ...
  private void tutorialSlideToMiddle(final boolean isLeftSide) {
    ...
    textView.postDelayed(new Runnable() {
      @Override public void run() {
        changeTextViewTypeFaceStyle(textView, Typeface.BOLD);
      }
    }, boldTextDelay);
  }
  ...
  protected void forceRippleAnimation(View view) {
    ...
    final RippleDrawable rippleDrawable =
        (RippleDrawable) ContextCompat.getDrawable(mContext, R.drawable.ripple_effect);

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      view.setBackground(rippleDrawable);
      rippleDrawable.setHotspot(mX_initial_position,
          mFab.getY()); // TODO: 1/17/17 fix hotspot to right side
    }
